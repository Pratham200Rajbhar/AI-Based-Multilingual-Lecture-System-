...
export default function SuperPolicies() {
  const [announcements, setAnnouncements] = useState([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ title: '', message: '', priority: 'medium', type: 'policy' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ title: '', message: '', priority: 'medium', type: 'policy' });
      setShowForm(false);
      fetchAnnouncements();
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this policy announcement?')) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-heading">Global Policies</h1>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-surface rounded-card shadow-card border border-border p-6 space-y-4">
          <h2 className="text-lg font-semibold text-heading">New Policy Announcement</h2>
          ...
          <button type="submit" className="btn-primary px-4 py-2 rounded-btn text-sm">Publish Policy</button>
        </form>
      )}

      <div className="space-y-4">
        {announcements.length === 0 ? (
          <div className="bg-surface rounded-card shadow-card border border-border p-12 text-center text-muted">No policies published</div>
        ) : (
          announcements.map(a => (
